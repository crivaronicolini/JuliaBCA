...
# function Mendenhall_weller(Za::Int, Zb::Int, ma::MassConcrete, mb::MassConcrete, E0::EnergyConcrete, impactparameter::LengthConcrete, x0::LengthConcrete, interaction_potential::Union{Type{Moliere}})
function Mendenhall_weller(Za::Int, Zb::Int, ma::MassConcrete, mb::MassConcrete, E0::EnergyConcrete, impactparameter::LengthConcrete, x0::Real, interaction_potential::Type{T} where {T<:InteractionPotential})
...
struct CollisionResult
  θ::AngleConcrete
  ψ::AngleConcrete
  ψ_recoil::AngleConcrete
  E_recoil::EnergyConcrete
  asymptotic_deflection::LengthConcrete
  normalized_distance_of_closest_approach::Real
end

struct CollisionGeometry
  ϕ_azimuthal::AngleConcrete
  impactparameter::LengthConcrete
  mfp::LengthConcrete
end
...
function distance_of_closest_approach(particle1::Particle, particle2::Particle, collisiongeometry::CollisionGeometry, options::Options)
  ...
  a = screening_length(Za, Zb, interaction_potential) |> u"μm"
...
function choose_collision_partner(particle::Particle, target::Target, collisiongeometry::CollisionGeometry, options::Options)
  ...
  recoil::Vec3{LengthConcrete} = Vec3(x + mfp * cosx - impactparameter * cosϕ * sinx,
...
function update_particle_energy!(particle::Particle, target::Target, distance_traveled::LengthConcrete, E_recoil::EnergyConcrete, x0::Real, strong_collision_Z::Int, strong_collision_index::Int, options::Options)
  # @debug "update_particle_energy!"
  #If particle energy  drops below zero before electronic stopping calcualtion, it produces NaNs
  @assert !isnan(E_recoil) "Numerical error: recoil EnergyConcrete is NaN. E0 = $(particle.E) Za = $(particle.Z) Ma = $(particle.m) x0 = $x0 Zb = $strong_collision_Z delta-x = $distance_traveled"
